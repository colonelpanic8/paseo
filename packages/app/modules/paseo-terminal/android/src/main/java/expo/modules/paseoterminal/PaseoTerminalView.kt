...
class PaseoTerminalView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  private val onSwipeRight by EventDispatcher()
  private var terminalHandle = 0L
  private val remoteBuffer = StringBuilder()
  private var fedBufferLength = 0
  ...
  var terminalKey: String = ""
    set(value) {
      ...
      field = value
      contentDescription = "paseo-terminal-$value"
      recreateTerminal()
    }

  var initialBuffer: String
    get() = remoteBuffer.toString()
    set(value) {
      if (!hasIncrementalOutput && value.contentEquals(remoteBuffer)) return
      remoteBuffer.clear()
      remoteBuffer.append(value)
      fedBufferLength = -1
      feedPendingBuffer()
    }
  ...
  var swipeGesturesEnabled: Boolean = false
  ...
  init {
    ...
    terminalCanvas.onSwipeLeft = {
      if (swipeGesturesEnabled) onSwipeLeft(emptyMap())
    }
    terminalCanvas.onSwipeRight = {
      if (swipeGesturesEnabled) onSwipeRight(emptyMap())
    }
  ...
  fun write(data: String) {
    if (data.isEmpty() || terminalHandle == 0L) return
    feedTerminal(data)
    hasIncrementalOutput = true
  }

  fun replaceBuffer(data: String) {
    initialBuffer = data
  }
  ...
  fun cleanup() {
    ...
    terminalCanvas.onSwipeLeft = null
    terminalCanvas.onSwipeRight = null
  ...
  private fun feedPendingBuffer() {
    if (terminalHandle == 0L || remoteBuffer.length == fedBufferLength) return
    if (fedBufferLength < 0 || fedBufferLength > remoteBuffer.length) {
      recreateTerminal()
      if (terminalHandle == 0L) return
    }
    val suffix = remoteBuffer.substring(fedBufferLength)
    if (suffix.isNotEmpty()) {
      feedTerminal(suffix)
    }
    fedBufferLength = remoteBuffer.length
    hasIncrementalOutput = false
  }

  private fun feedTerminal(data: String) {
    // Paseo's daemon owns terminal-query replies. Feeding libghostty-vt output
    // back to the PTY here would duplicate those replies.
    GhosttyBridge.nativeFeed(terminalHandle, data.toByteArray(Charsets.UTF_8))
  ...
  private fun requestKeyboardFocus() {
    if (!inputView.requestFocus()) return
    onFocus(emptyMap())
