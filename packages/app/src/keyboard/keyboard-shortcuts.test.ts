...
describe("keyboard-shortcuts", () => {
  const matchingCases: MatchingShortcutCase[] = [
    ...
    {
      name: "matches Cmd+Shift+I to select workspace isolation on mac",
      event: { key: "I", code: "KeyI", metaKey: true, shiftKey: true },
      context: { isMac: true, focusScope: "message-input" },
      action: "workspace.isolation.pick",
    },
    {
      name: "matches Ctrl+Shift+B to select the starting ref on non-mac",
      event: { key: "B", code: "KeyB", ctrlKey: true, shiftKey: true },
      context: { isMac: false, focusScope: "message-input" },
      action: "workspace.ref.pick",
    },
    {
      name: "matches Cmd+Shift+M to select the model on mac",
      event: { key: "M", code: "KeyM", metaKey: true, shiftKey: true },
      context: { isMac: true, focusScope: "message-input" },
      action: "message-input.model.pick",
    },
    {
      name: "matches Ctrl+Shift+E to select thinking effort on non-mac",
      event: { key: "E", code: "KeyE", ctrlKey: true, shiftKey: true },
      context: { isMac: false, focusScope: "message-input" },
      action: "message-input.thinking.pick",
    },
    {
      name: "matches Cmd+Shift+A to select agent mode on mac",
      event: { key: "A", code: "KeyA", metaKey: true, shiftKey: true },
      context: { isMac: true, focusScope: "message-input" },
      action: "message-input.mode.pick",
    },
    {
      name: "matches Ctrl+Alt+F to toggle fast mode on non-mac",
      event: { key: "f", code: "KeyF", ctrlKey: true, altKey: true },
      context: { isMac: false, focusScope: "message-input" },
      action: "message-input.fast-mode.toggle",
    },
    {
      name: "matches Cmd+Shift+H to select the host on mac",
  ...
  const helpCases: HelpSectionCase[] = [
    {
      ...
      expectedKeys: {
        ...
        "select-workspace-isolation": ["mod", "shift", "I"],
        "select-starting-ref": ["mod", "shift", "B"],
        "select-model": ["mod", "shift", "M"],
