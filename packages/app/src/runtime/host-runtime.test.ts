...
describe("HostRuntimeStore", () => {
  ...
  it("serializes host removal after a pending appearance write", async () => {
    const host = makeHost({ serverId: "srv_appearance" });
    const storage = createMemoryHostRuntimeStorage();
    await storage.setItem("@paseo:daemon-registry", JSON.stringify([host]));
    await storage.setItem("@paseo:e2e", "1");
    ...
    const color = store.setHostColor("srv_appearance", "teal");
    const remove = store.removeHost("srv_appearance");
    await Promise.resolve();
    expect(writeCount).toBe(1);

    firstWrite.resolve();
    await Promise.all([color, remove]);

    expect(store.getHosts()).toEqual([]);
    expect(JSON.parse((await storage.getItem("@paseo:daemon-registry")) ?? "[]")).toEqual([]);
  });

  it("continues registry mutations after an appearance write fails", async () => {
    const host = makeHost({ serverId: "srv_appearance" });
    const storage = createMemoryHostRuntimeStorage();
    await storage.setItem("@paseo:daemon-registry", JSON.stringify([host]));
    await storage.setItem("@paseo:e2e", "1");
    ...
    const color = store.setHostColor("srv_appearance", "teal");
    const rename = store.renameHost("srv_appearance", "Office");
    await expect(color).rejects.toThrow("disk full");
    await rename;

    expect(store.getHosts()[0]?.label).toBe("Office");
    expect(JSON.parse((await storage.getItem("@paseo:daemon-registry")) ?? "[]")[0]?.label).toBe(
      "Office",
    );
    store.syncHosts([]);
  });
