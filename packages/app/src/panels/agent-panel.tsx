...
function useAgentPanelDescriptor(
  ...
  const descriptorState = useSessionStore(
    useShallow((state) => {
      const descriptor = selectAgentPanelDescriptorState(state, context.serverId, target.agentId);
  ...
  return {
    label: label ?? "",
    subtitle: descriptorState.summary ? `${purposePresentation.subtitle} · ${subtitle}` : subtitle,
    tooltip: descriptorState.summary
