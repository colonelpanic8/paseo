...
export function resolvePaseoPaths(
  ...
  // Cache each complete set of resolution inputs for the process lifetime.
...
export function persistPaseoLayoutSelection(paths: PaseoPaths): void {
  if (paths.layout !== "xdg") return;
  const markerPath = path.join(paths.home, XDG_LAYOUT_MARKER);
  if (!existsSync(markerPath)) writePrivateFileAtomicSync(markerPath, "1\n");
}
