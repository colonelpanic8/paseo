...
describe("resolvePaseoPaths", () => {
  ...
  test("different XDG environments resolve independently", () => {
    const home = makeHome();
    ...
    const after = resolvePaseoPaths(env, "linux", home);
    expect(after.config).toBe(path.join(home, "config-after", "paseo"));
    expect(after.data).toBe(path.join(home, "data-after", "paseo"));
