...
import type { AgentFailure } from "@getpaseo/protocol/messages";
...
function isAuthenticationFailure(
  ...
  return /auth(?:entication)?(?:[_\s-]+)(?:failed|required)/i.test(
    `${event.error}\n${event.diagnostic ?? ""}`,
  );
}
...
export class AgentManager {
  ...
  private async onStreamTurnFailed(params: {
    ...
    agent.lastFailure = {
      kind: isAuthenticationFailure(event) ? "authentication_required" : "provider_error",
      message: event.error,
