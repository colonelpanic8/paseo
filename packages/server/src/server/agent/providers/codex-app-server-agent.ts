...
export class CodexAppServerAgentSession implements AgentSession, AgentRealtimeVoiceSession {
  ...
  private async establishConnection(): Promise<void> {
    ...
    this.unsubscribeTransportClose = client.onClose((reason) => {
  ...
  async close(): Promise<void> {
    ...
    this.pendingForegroundTurnIdentification?.resolve(null);
    this.pendingForegroundTurnIdentification = null;
    // disposeClient() synchronously notifies onClose, so realtime subscribers
    // still get a terminal event before we drop them below.
    await this.disposeClient();
