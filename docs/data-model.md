...
| `summary`                 | `string?` (nullable)                     | Generated one-line description of the agent's current purpose                                                                                                                                                                                                                                                                                                                       |
| `summaryUpdatedAt`        | `string?` (ISO 8601)                     | When the purpose summary was last generated; persisted for cadence and legacy timestamp-boundary fallback but not sent on the wire                                                                                                                                                                                                                                                  |
| `summaryCursor`           | `{ epoch: string, seq: number }?`        | Last timeline position included in the purpose summary; persisted so turns completed during generation remain eligible for the next update, but not sent on the wire                                                                                                                                                                                                                |
| `summaryTurnsSinceUpdate` | `number?`                                | Completed turns since the purpose summary was last written; incremented on `turn_completed` only (not failed/canceled turns) and persisted so the refresh cadence survives a daemon restart, but not sent on the wire                                                                                                                                                               |
...
`agents.metadataGeneration.providers` controls the preferred structured-generation fallback order for daemon-side metadata tasks such as commit messages, PR text, branch names, workspace titles, and rolling agent purpose summaries. Entries are tried first in the configured order, then Paseo falls through to dynamically discovered defaults and finally the current selection when available.
...
| `autoArchivedChangeRequestUrl` | `string \| null`                                             | Merged change request already acknowledged by auto-archive or manual restore. Restore records its fresh forge baseline in the background; only the restored workspace is protected while that read is pending. |
...
| `snoozeStatus`                 | `{ snoozedAt, snoozedUntil } \| null` (ISO 8601)             | User-set quiet period. Never auto-cleared: "is it snoozed now" is this pair resolved against the clock. Null means "not snoozed". See **Snoozed** in [glossary.md](glossary.md).              |
