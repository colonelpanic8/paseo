...
Only `en` is registered at init. Other locales load the first time `changeLanguage` selects them, through the synchronous backend in `packages/app/src/i18n/i18next.ts`. To register a new locale, add a `case` with a string-literal `require()` path there; do not import the resource statically. Expo's Metro config disables inline requires, so a static import evaluates that bundle on every cold start (#4818). The load is synchronous so the first render in a non-English locale does not show English.
