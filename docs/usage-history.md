...
The chart keeps its time axis chronological. Ordered sorting criteria apply to the table without changing the chart. Series colors stay attached to groups when sorting changes. The chart stacks its areas in series order from the baseline up, so a day's bands add up to that day's total and no series is hidden behind another; the scale reaches the largest daily sum. A band is drawn between two shared boundaries — cumulative curve i and cumulative curve i + 1 — retraced with the same builder, so neighbours abut exactly instead of drifting apart where two independently built curves disagree. Line shape is a chart control: smooth (monotone cubic, the default), linear, or step. All shapes pass through the recorded daily values; step changes value halfway between days, on the same boundary as the hover strips. Provider summaries remain below the graph, and failed sources and unpriced activity keep their coverage notes.
