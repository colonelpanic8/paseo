...
The scan enumerates a transcript home for **every configured provider**, not just the three defaults. A user can run several accounts of the same kind by extending a built-in provider with its own `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, or `XDG_DATA_HOME` (see [custom-providers.md](custom-providers.md)), and each of those homes holds its own transcripts. Reading only `process.env` and the default home is the bug this replaced: on a machine with three extra homes it reported less than half the real Codex usage, and looked healthy while doing it.

The base kind is resolved by following `extends` transitively; anything that does not land on `claude`, `codex`, or `opencode` keeps no token transcripts and is skipped. Homes are deduplicated by `realpath`, because two configured providers may point at one directory and would otherwise double count every token in it. Built-in defaults are always scanned, even when the provider is disabled, since past usage is still real. Each scan re-reads the current configuration, so adding a provider takes effect without a daemon restart.

OpenCode homes resolve through `XDG_DATA_HOME` (`~/.local/share/opencode` when unset) rather than a dedicated variable, and the scan reads every `opencode*.db` in that directory — the filename carries the release channel — straight from the `message` table. They bypass the file scan and its cache: each read queries SQLite directly, and a database that cannot be read fails its home instead of scanning as empty.
...
- **OpenCode rows carry their own turn deltas.** Each assistant message sums into its session's rollup exactly, so every row counts once keyed by message id. `input` already excludes both cached portions — a session's `tokens_cache_read` can exceed its `tokens_input` — and maps straight onto uncached input. The row's `cost` is provider-reported; free-tier turns report zero.
...
The chart keeps its time axis chronological. The chart opens with no grouping selected — one total series — and the table opens sorted by day, newest first. Ordered sorting criteria apply to the table without changing the chart. Series colors stay attached to groups when sorting changes. The chart layers its areas, with a scale based on the largest individual series-day. Line shape is a chart control: smooth (monotone cubic, the default), linear, or step. All shapes pass through the recorded daily values; step changes value halfway between days, on the same boundary as the hover strips. Provider summaries remain below the graph, and failed sources and unpriced activity keep their coverage notes.
