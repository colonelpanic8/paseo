...
`$PASEO_HOME/usage-history/scan-cache.json` memoizes parsed records per file, keyed by `(size, mtime, provider)`, with the byte offset where parsing stopped. Transcripts are append-only, so a file that grew resumes from that offset after a 64-byte guard hash confirms the tail is unchanged; anything else re-parses from byte zero. The resumed base and the appended lines are deduped as one pass, so a subagent message whose real `output_tokens` block landed after the previous scan replaces the partial count already cached. The cache is per file rather than per day so changing the time zone invalidates nothing. Scans run serially. Any active session dirties the cache on every scan, so it is written five seconds after the last scan that changed it rather than inline, and flushed when the daemon closes. Identical pending requests share a result; at most eight distinct requests are admitted, with excess requests rejected for retry. Entries older than 90 days are pruned. A corrupt cache costs one cold scan, never an error. Bump `USAGE_SCAN_CACHE_VERSION` when a parse or dedupe rule changes, or cached files keep their old numbers until they grow.

`$PASEO_HOME/usage-history/model-rates.json` is the last fetched rate table. It is refreshed after 24 hours, or on the page's refresh action subject to a 60 second floor. Offline, the page keeps working from the snapshot and reports `pricing.status: "cached"`. A fetch that fails is not retried for five minutes, because each attempt can hold a scan for its 10 second timeout.
...
A cold scan costs minutes, so these queries are the one exception to the app's refetch-on-mount default: a remount inside the five-minute stale window reuses the cached answer, and the refresh action is the only thing that re-reads. The requested window is anchored to the day the page opened and re-anchored once local midnight passes, so a page left open overnight keeps asking for today instead of pinning yesterday. Per-host results are combined inside the query layer, which structurally shares them, so the host list keeps its identity until a host's answer changes: merging and grouping run once per answer rather than once per render, and changing the metric or reversing a sort only reorders rows the page already has.
...
`merge.ts` folds the payloads into one report and `derive.ts` does the arithmetic, taking one entry per contributing host; a single host is the length-1 case, not a second code path. Provider summary identity is `(serverId, providerId)` so you can inspect each host’s configured sources separately. Labels and ids carry the host name only once more than one host contributes, so a single-host page reads exactly as it did.
...
The graph uses the full settings content width above the provider summary. Chart and table grouping are independent. Both offer Host, Provider, and Model; the table also offers calendar periods. Only observed combinations produce rows; selecting no dimensions gives one total. The Provider toggle groups on `(provider, providerId)` across hosts: the built-in `codex` on two machines is one row, and adding Host splits it. Transcripts carry nothing that identifies an account, so two accounts that should stay apart need two configured provider ids. The grouped view consumes only buckets retained by the existing source deduplication, never the raw multi-host responses.

The chart keeps its time axis chronological. Ordered sorting criteria apply to the table without changing the chart. Series colors stay attached to groups when sorting changes. Day/week/month is one page-level control, always applied: it sets how wide a chart column is and how the table groups its time rows, so both read the same axis. Chart and table choose their own dimensions (host, provider, model), but never their own period.

The chart stacks its areas in series order from the baseline up, so a period's bands add up to that period's total and no series is hidden behind another; the scale reaches the largest period total. A band is drawn between two shared boundaries — cumulative curve i and cumulative curve i + 1 — retraced from the same points, so neighbours abut exactly instead of drifting apart where two independently built curves disagree. Provider summaries remain below the graph, and failed sources and unpriced activity keep their coverage notes.
