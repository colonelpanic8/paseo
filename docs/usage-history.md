...
RPC: `provider.usage_history.read.request` / `.response`, gated on `server_info.features.providerUsageHistory`. Buckets are `(day, provider, providerId, model)` in the client's IANA time zone, so a turn lands on the day the user experienced it.
